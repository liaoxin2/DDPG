...
class A_functions:
    ...
    def A_pinv(self, vec):
        ...
        temp[:, : tuple(singulars.shape)[0]] = temp[:, : tuple(singulars.shape)[0]] * factors
...
class CS(A_functions):
    ...
    def V(self, vec):
        ...
        patches = paddle.zeros(shape=[vec.shape[0], self.channels * self.y_dim**2, self.ratio**2])
        ...
        patches = paddle.matmul(x=self.V_small, y=patches.reshape(-1, self.ratio**2, 1)).reshape(
    ...
    def Vt(self, vec):
        patches = vec.clone().reshape(tuple(vec.shape)[0], self.channels, self.img_dim, self.img_dim)
        patches = patches.unfold(axis=2, size=self.ratio, step=self.ratio).unfold(
            axis=3, size=self.ratio, step=self.ratio
        )
        patches = patches.contiguous().reshape(tuple(vec.shape)[0], self.channels, -1, self.ratio**2)
        patches = paddle.matmul(x=self.Vt_small, y=patches.reshape(-1, self.ratio**2, 1)).reshape(
            tuple(vec.shape)[0], self.channels, -1, self.ratio**2
        )
        recon = paddle.zeros(shape=[tuple(vec.shape)[0], self.channels * self.img_dim**2])
        recon[:, : self.channels * self.y_dim**2 * self.cs_size] = (
            patches[:, :, :, : self.cs_size].contiguous().reshape(tuple(vec.shape)[0], -1)
        )
        recon[:, self.channels * self.y_dim**2 * self.cs_size :] = (
            patches[:, :, :, self.cs_size :].contiguous().reshape(tuple(vec.shape)[0], -1)
        )
    ...
    def add_zeros(self, vec):
        ...
        temp = paddle.zeros(shape=(tuple(vec.shape)[0], self.channels * self.img_dim**2))
...
def gray2color(x):
    ...
    return paddle.stack(x=(x * 0.3333 / base, x * 0.3334 / base, x * 0.3333 / base), axis=1)
...
class GeneralA(A_functions):
    def mat_by_vec(self, M, v):
        ...
        return paddle.matmul(x=M, y=v.view(tuple(v.shape)[0], vshape, 1)).view(tuple(v.shape)[0], tuple(M.shape)[0])
...
class WalshHadamardCS(A_functions):
    def fwht(self, vec):
        ...
        while h < self.img_dim**2:
            ...
            a[:, :, :, h : 2 * h] = b[:, :, :, :h] - b[:, :, :, h : 2 * h]
            h *= 2
        a = a.reshape(tuple(vec.shape)[0], self.channels, self.img_dim**2) / self.img_dim
        return a
    ...
    def V(self, vec):
        temp = paddle.zeros(shape=[tuple(vec.shape)[0], self.channels, self.img_dim**2])
        temp[:, :, self.perm] = vec.clone().reshape(tuple(vec.shape)[0], -1, self.channels).transpose(perm=[0, 2, 1])
        return self.fwht(temp).reshape(tuple(vec.shape)[0], -1)

    def Vt(self, vec):
        return self.fwht(vec.clone())[:, :, self.perm].transpose(perm=[0, 2, 1]).reshape(tuple(vec.shape)[0], -1)
    ...
    def add_zeros(self, vec):
        out = paddle.zeros(shape=[tuple(vec.shape)[0], self.channels * self.img_dim**2])
        out[:, : self.channels * self.img_dim**2 // self.ratio] = vec.clone().reshape(tuple(vec.shape)[0], -1)
        return out
...
class Inpainting(A_functions):
    def __init__(self, channels, img_dim, missing_indices, device):
        self.channels = channels
        self.img_dim = img_dim
        self._singulars = paddle.ones(shape=channels * img_dim**2 - tuple(missing_indices.shape)[0]).to(device)
        self.missing_indices = missing_indices
        self.kept_indices = (
            paddle.to_tensor(
                data=[i for i in range(channels * img_dim**2) if i not in missing_indices],
                dtype="float32",
            )
            .to(device)
            .astype(dtype="int64")
...
class Inpainting_sst(A_functions):
    def __init__(self, channels, img_lat, img_lon, missing_indices, device):
        ...
        self._singulars = paddle.ones(shape=channels * img_lat * img_lon - len(missing_indices)).to(device)
        self.missing_indices = missing_indices
    ...
    def V(self, vec):
        ...
        return out.reshape([vec.shape[0], -1, self.channels]).transpose(perm=[0, 2, 1]).reshape([vec.shape[0], -1])
    ...
    def add_zeros(self, vec):
        temp = paddle.zeros(shape=(tuple(vec.shape)[0], self.channels * self.img_lat * self.img_lon))
...
class SuperResolution(A_functions):
    def __init__(self, channels, img_dim, ratio, device):
        ...
        self.ratio = ratio
        A = paddle.to_tensor(data=[[1 / ratio**2] * ratio**2], dtype="float32").to(device)
        tmp_u, tmp_s, tmp_v = paddle.linalg.svd(x=A, full_matrices=not False)
    ...
    def V(self, vec):
        temp = vec.clone().reshape(tuple(vec.shape)[0], -1)
        patches = paddle.zeros(shape=[tuple(vec.shape)[0], self.channels, self.y_dim**2, self.ratio**2])
        patches[:, :, :, 0] = temp[:, : self.channels * self.y_dim**2].view(tuple(vec.shape)[0], self.channels, -1)
    ...
    def add_zeros(self, vec):
        reshaped = vec.clone().reshape(tuple(vec.shape)[0], -1)
        temp = paddle.zeros(shape=(tuple(vec.shape)[0], tuple(reshaped.shape)[1] * self.ratio**2))
        temp[:, : tuple(reshaped.shape)[1]] = reshaped
        return temp
...
class Colorization(A_functions):
    def __init__(self, img_dim, device):
        self.channels = 3
        self.img_dim = img_dim
        A = paddle.to_tensor(data=[[0.3333, 0.3334, 0.3333]], dtype="float32").to(device)
        tmp_u, tmp_s, tmp_v = paddle.linalg.svd(x=A, full_matrices=not False)
        self.U_small, self.singulars_small, self.V_small = (
            tmp_u,
            tmp_s,
    ...
    def V(self, vec):
        needles = vec.clone().reshape(tuple(vec.shape)[0], self.channels, -1).transpose(perm=[0, 2, 1])
        needles = paddle.matmul(x=self.V_small, y=needles.reshape(-1, self.channels, 1)).reshape(
            tuple(vec.shape)[0], -1, self.channels
        )
    ...
    def Vt(self, vec):
        needles = vec.clone().reshape(tuple(vec.shape)[0], self.channels, -1).transpose(perm=[0, 2, 1])
        needles = paddle.matmul(x=self.Vt_small, y=needles.reshape(-1, self.channels, 1)).reshape(
            tuple(vec.shape)[0], -1, self.channels
        )
...
class SRConv(A_functions):
    def mat_by_img(self, M, v, dim):
        return paddle.matmul(x=M, y=v.reshape(tuple(v.shape)[0] * self.channels, dim, dim)).reshape(
            tuple(v.shape)[0], self.channels, tuple(M.shape)[0], dim
        )

    def img_by_mat(self, v, M, dim):
        return paddle.matmul(x=v.reshape(tuple(v.shape)[0] * self.channels, dim, dim), y=M).reshape(
            tuple(v.shape)[0], self.channels, dim, tuple(M.shape)[1]
        )
    ...
    def __init__(self, kernel, channels, img_dim, device, stride=1):
        ...
        for i in range(stride // 2, img_dim + stride // 2, stride):
            for j in range(i - tuple(kernel.shape)[0] // 2, i + tuple(kernel.shape)[0] // 2):
                j_effective = j
                if j_effective < 0:
                    j_effective = -j_effective - 1
                if j_effective >= img_dim:
                    j_effective = img_dim - 1 - (j_effective - img_dim)
                A_small[i // stride, j_effective] += kernel[j - i + tuple(kernel.shape)[0] // 2]
        ...
        self._perm = (
            paddle.to_tensor(
                data=[(self.img_dim * i + j) for i in range(self.small_dim) for j in range(self.small_dim)]
                + [(self.img_dim * i + j) for i in range(self.small_dim) for j in range(self.small_dim, self.img_dim)],
                dtype="float32",
            )
            .to(device)
            .astype(dtype="int64")
    ...
    def V(self, vec):
        temp = paddle.zeros(shape=[tuple(vec.shape)[0], self.img_dim**2, self.channels])
        temp[:, self._perm, :] = vec.clone().reshape(tuple(vec.shape)[0], self.img_dim**2, self.channels)[
    ...
    def Vt(self, vec):
        ...
        temp = self.img_by_mat(temp, self.V_small, self.img_dim).reshape(tuple(vec.shape)[0], self.channels, -1)
        temp[:, :, : tuple(self._perm.shape)[0]] = temp[:, :, self._perm]
        temp = temp.transpose(perm=[0, 2, 1])
        return temp.reshape(tuple(vec.shape)[0], -1)

    def U(self, vec):
        temp = paddle.zeros(shape=[tuple(vec.shape)[0], self.small_dim**2, self.channels])
        temp[:, : self.small_dim**2, :] = vec.clone().reshape(tuple(vec.shape)[0], self.small_dim**2, self.channels)
        temp = temp.transpose(perm=[0, 2, 1])
    ...
    def Ut(self, vec):
        ...
        temp = self.img_by_mat(temp, self.U_small, self.small_dim).reshape(tuple(vec.shape)[0], self.channels, -1)
        temp = temp.transpose(perm=[0, 2, 1])
        return temp.reshape(tuple(vec.shape)[0], -1)
...
class SRConv_NonSquare(A_functions):
    ...
    def __init__(self, kernel, channels, img_height, img_width, device, stride=1):
        ...
        self._singulars = (
            self.singulars_small_h.reshape([small_height, 1]) @ self.singulars_small_w.reshape([1, small_width])
        ).reshape([-1])
        ...
        self._perm = paddle.to_tensor(
            ...
            + [
                self.img_width * i + j
                for i in range(self.small_height)
                for j in range(self.small_width, self.img_width)
            ],
            dtype="int64",
        )
    ...
    def V(self, vec):
        ...
        temp[:, self._perm, :] = vec.reshape([vec.shape[0], self.img_height * self.img_width, self.channels])[
            :, : self._perm.shape[0], :
        ]
        temp[:, self._perm.shape[0] :, :] = vec.reshape(
            [vec.shape[0], self.img_height * self.img_width, self.channels]
        )[:, self._perm.shape[0] :, :]
    ...
    def Vt(self, vec):
        ...
        temp = self.img_by_mat(temp, self.V_small_w.transpose([1, 0]), self.img_height, self.img_width).reshape(
    ...
    def U(self, vec):
        ...
        temp[:, : self.small_height * self.small_width, :] = vec.reshape(
            [vec.shape[0], self.small_height * self.small_width, self.channels]
        )
        temp = temp.transpose([0, 2, 1])
        out = self.mat_by_img(self.U_small_h, temp, self.small_height, self.small_width)
        out = self.img_by_mat(out, self.U_small_w.transpose([1, 0]), self.small_height, self.small_width).reshape(
            [vec.shape[0], -1]
        )
        return out
    ...
    def Ut(self, vec):
        ...
        temp = self.img_by_mat(temp, self.U_small_w, self.small_height, self.small_width).reshape(
            [vec.shape[0], self.channels, -1]
        )
        # Inverse permutation
        temp = temp.transpose([0, 2, 1])
...
class Deblurring(A_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, ZERO=0.03):
        ...
        self._singulars, self._perm = paddle.sort(descending=True, x=self._singulars), paddle.argsort(
    ...
    def V(self, vec):
        ...
        out = self.img_by_mat(out, self.V_small.transpose([1, 0])).reshape(tuple(vec.shape)[0], -1)
        return out

    def Vt(self, vec):
        temp = self.mat_by_img(self.V_small.transpose([1, 0]), vec.clone())
        temp = self.img_by_mat(temp, self.V_small).reshape(tuple(vec.shape)[0], self.channels, -1)
    ...
    def U(self, vec):
        ...
        out = self.img_by_mat(out, self.U_small.transpose([1, 0])).reshape(tuple(vec.shape)[0], -1)
        return out

    def Ut(self, vec):
        temp = self.mat_by_img(self.U_small.transpose([1, 0]), vec.clone())
        temp = self.img_by_mat(temp, self.U_small).reshape(tuple(vec.shape)[0], self.channels, -1)
        temp = temp[:, :, self._perm].transpose(perm=[0, 2, 1])
        return temp.reshape(tuple(vec.shape)[0], -1)
...
class Deblurring2D(A_functions):
    ...
    def __init__(self, kernel1, kernel2, channels, img_dim, device):
        ...
        for i in range(img_dim):
            for j in range(i - tuple(kernel1.shape)[0] // 2, i + tuple(kernel1.shape)[0] // 2):
                if j < 0 or j >= img_dim:
                    continue
                A_small1[i, j] = kernel1[j - i + tuple(kernel1.shape)[0] // 2]
        A_small2 = paddle.zeros(shape=[img_dim, img_dim])
        for i in range(img_dim):
            for j in range(i - tuple(kernel2.shape)[0] // 2, i + tuple(kernel2.shape)[0] // 2):
                if j < 0 or j >= img_dim:
                    continue
    ...
    def V(self, vec):
        ...
        out = self.img_by_mat(out, self.V_small2.transpose([1, 0])).reshape(tuple(vec.shape)[0], -1)
        return out
    ...
    def Vt(self, vec):
        ...
        temp = self.img_by_mat(temp, self.V_small2).reshape(tuple(vec.shape)[0], self.channels, -1)
        temp = temp[:, :, self._perm].transpose(perm=[0, 2, 1])
        return temp.reshape(tuple(vec.shape)[0], -1)
    ...
    def U(self, vec):
        ...
        out = self.img_by_mat(out, self.U_small2.transpose([1, 0])).reshape(tuple(vec.shape)[0], -1)
        return out
    ...
    def Ut(self, vec):
        ...
        temp = self.img_by_mat(temp, self.U_small2).reshape(tuple(vec.shape)[0], self.channels, -1)
        temp = temp[:, :, self._perm].transpose(perm=[0, 2, 1])
        return temp.reshape(tuple(vec.shape)[0], -1)
