...
def verify_str_arg(value, arg_name, valid_options):
    if not isinstance(value, str):
        raise TypeError(f"Expected argument '{arg_name}' to be a string, but got {type(value)}.")
    if value not in valid_options:
        raise ValueError(f"Invalid value '{value}' for argument '{arg_name}'. "
                         f"Supported options are: {iterable_to_str(valid_options)}.")
...
class LSUN(VisionDataset):
    ...
    def _verify_classes(self, classes):
        ...
        try:
            ...
            for c in classes:
                verify_str_arg(
                    c, custom_msg=msg_fmtstr.format(type(c))
                )
                c_short = c.split("_")
