...
class CelebA(VisionDataset):
    ...
    def __init__(
        ...
        if split.lower() == "train":
            ...
            raise ValueError(
                'Wrong split entered! Please use split="train" or split="valid" or split="test"'
            )
        with open(
        ...
        self.identity = paddle.to_tensor(data=self.identity[mask].values)
        self.bbox = paddle.to_tensor(data=self.bbox[mask].values)
        self.landmarks_align = paddle.to_tensor(data=self.landmarks_align[mask].values)
        self.attr = paddle.to_tensor(data=self.attr[mask].values)
