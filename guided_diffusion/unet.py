...
class AttentionPool2d(paddle.nn.Layer):
    ...
    def __init__(
        ...
        super().__init__()
        self.positional_embedding = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.randn(shape=[embed_dim, spacial_dim**2 + 1])
            / embed_dim**0.5
        )
...
class SuperResModel(UNetModel):
    ...
    def forward(self, x, timesteps, low_res=None, **kwargs):
        _, _, new_height, new_width = tuple(x.shape)
        upsampled = paddle.nn.functional.interpolate(
            x=low_res, size=(new_height, new_width), mode="bilinear"
        )
        x = paddle.concat(x=[x, upsampled], axis=1)
        return super().forward(x, timesteps, **kwargs)


class EncoderUNetModel(paddle.nn.Layer):
    """
    ...
    def __init__(
        ...
        if pool == "adaptive":
            self.out = paddle.nn.Sequential(
                normalization(ch),
                paddle.nn.Silu(),
                paddle.nn.AdaptiveAvgPool2D(output_size=(1, 1)),
            ...
            self.out = paddle.nn.Sequential(
                paddle.nn.Linear(in_features=self._feature_size, out_features=2048),
                paddle.nn.ReLU(),
                paddle.nn.Linear(in_features=2048, out_features=self.out_channels),
            )
    ...
    def forward(self, x, timesteps):
        ...
        for module in self.input_blocks:
            ...
            if self.pool.startswith("spatial"):
                results.append(h.astype(x.dtype).mean(axis=(2, 3)))
        h = self.middle_block(h, emb)
        if self.pool.startswith("spatial"):
            results.append(h.astype(x.dtype).mean(axis=(2, 3)))
